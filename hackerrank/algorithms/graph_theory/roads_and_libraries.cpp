...
template<typename T>
T evaluate_minimal_cost(T towns_count, T library_cost, T road_cost,
                        std::unordered_map<T, std::set<T>>& roads)
{
    ...
    return library_groups * library_cost + road_cost * (towns_count -
                                                        library_groups);
}
...
int main()
{
    ...
    while (size-- != 0) {
        ...
        std::cout << evaluate_minimal_cost(town_count, library_cost, road_cost,
                                           roads) << std::endl;
    }
